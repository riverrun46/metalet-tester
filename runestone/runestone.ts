...
export class Runestone {
  ...
  static decipher(transaction: RunestoneTx): Option<Artifact> {
    ...
    const etching: Option<Etching> = etchingFlag
      ? (() => {
          const divisibility = Tag.take(
            ...
            1,
            ([value]): Option<u8> =>
              u128.tryIntoU8(value).andThen<u8>((value) => (value <= MAX_DIVISIBILITY ? Some(value) : None)),
          )
    ...
    const pointer = Tag.take(
      ...
      1,
      ([value]): Option<u32> =>
        u128.tryIntoU32(value).andThen((value) => (value < transaction.vout.length ? Some(value) : None)),
    )
