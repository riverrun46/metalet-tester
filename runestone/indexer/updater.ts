...
export class RuneUpdater implements RuneBlockIndex {
  ...
  async indexRunes(tx: UpdaterTx, txIndex: number): Promise<void> {
    ...
    const allocated: Map<string, RuneBalance>[] = [...new Array(tx.vout.length)].map(() => new Map())
    ...
    if (optionArtifact.isSome()) {
      ...
      if (optionMint.isSome()) {
        ...
        if (optionAmount.isSome()) {
          ...
          unallocatedBalance.amount = u128.checkedAddThrow(u128(unallocatedBalance.amount), u128(amount))
        }
      }
      ...
      if (isRunestone(artifact)) {
        ...
        for (const { id, amount, output } of [...runestone.edicts]) {
          ...
          if (Number(output) === tx.vout.length) {
            ...
            allocate(
              amount !== 0n && amount < u128(maybeBalance.amount) ? amount : u128(maybeBalance.amount),
              Number(output),
            )
          }
        }
    ...
    if (optionArtifact.isSome() && !isRunestone(optionArtifact.unwrap())) {
      for (const balance of unallocated.values()) {
        ...
        currentBalance.amount = u128.checkedAddThrow(u128(currentBalance.amount), u128(balance.amount))
      }
    } else {
    ...
    for (const [vout, balances] of allocated.entries()) {
      ...
      const etchingByRuneId = new Map(this.etchings.map((etching) => [RuneLocation.toString(etching.runeId), etching]))
  ...
  private async mint(id: RuneLocation, txid: string): Promise<Option<bigint>> {
    ...
    const end =
      endRelative !== null || endAbsolute !== null ? Math.max(endRelative ?? -Infinity, endAbsolute ?? -Infinity) : null
  ...
  async txCommitsToRune(tx: UpdaterTx, rune: Rune): Promise<boolean> {
    ...
    for (const input of tx.vin) {
      ...
      const offset = witnessStack.length >= 2 && lastWitnessElement[0] === TAPROOT_ANNEX_PREFIX ? 3 : 2
      ...
      for (const instruction of instructions) {
        ...
        const confirmations = u128.checkedSubThrow(u128(this.block.height), u128(commitTxHeight)) + 1n
        ...
        ...(terms.isSome()
          ? {
              terms: (() => {
                ...
                return {
                  ...(unwrappedTerms.amount.isSome() ? { amount: unwrappedTerms.amount.unwrap() } : {}),
